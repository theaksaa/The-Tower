...
{
    private const string RunOverviewMusicPath = AudioPaths.RunOverviewMusic;
    ...
    private void ConfigurePressedButtonFeedback(Button button, bool applyDisabledTint = true, bool swapPressedSprite = true)
    {
        ...
        feedback.Initialize(
            ...
            shouldApplyDisabledTint: applyDisabledTint,
            shouldTintLabelWhenDisabled: applyDisabledTint,
            shouldTintChildGraphicsWhenDisabled: applyDisabledTint,
            shouldSwapPressedSprite: swapPressedSprite);
    }
    ...
    private void ConfigureShopUi()
    {
        ...
        for (var index = 0; index < shopItems.Count; index++)
        {
            ...
            var pressedFeedback = item.Button.GetComponent<global::PressedButtonFeedback>();
    ...
    {
        return string.Equals((config?.type ?? string.Empty).Trim(), "move", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsItemShopItem(ShopItemConfig config)
    {
        return string.Equals((config?.type ?? string.Empty).Trim(), "item", StringComparison.OrdinalIgnoreCase);
    }
